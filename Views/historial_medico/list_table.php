...
<body class="homepage bg-light">
  ...
  <div class="p-4 shadow-sm" style="background-color: #ADB8B2; border-radius: 15px;">
    <!-- Título -->
    <h1 class="text-center mb-4" style="color:rgb(42, 42, 42); font-weight: bold;">Listado de Historiales Médicos</h1>

    <!-- Botón Crear Historial -->
    <div class="text-end mb-3">
      <a href="/DrVet/Views/historial_medico/form.php" class="btn btn-success" style="background-color: #869B8F; border-color:rgb(255, 255, 255); color: #fff;">Crear Nuevo Historial</a>
    </div>
        ...
        <?php if (!empty($historiales)): ?>
          <?php foreach ($historiales as $historial): ?>
            <tr>
              ...
              <td>
                <a href="/DrVet/Views/historial_medico/form.php?id=<?= $historial['ID_HISTORIAL'] ?>" class="btn btn-primary btn-sm">Editar</a>
                <a href="/DrVet/Views/historial_medico/delete.php?id=<?= $historial['ID_HISTORIAL'] ?>" class="btn btn-danger btn-sm" onclick="return confirm('¿Estás seguro de que deseas eliminar este historial?');">Eliminar</a>
              </td>
            </tr>
          <?php endforeach; ?>
        <?php else: ?>
