...
<body class="homepage bg-light">
  ...
  <div class="p-4 shadow-sm" style="background-color: #ADB8B2; border-radius: 15px;">
    <!-- Título -->
    <h1 class="text-center mb-4" style="color:rgb(42, 42, 42); font-weight: bold;">Listado de Inventarios</h1>

    <!-- Botón Crear Inventario -->
    <div class="text-end mb-3">
      <a href="/DrVet/Views/inventario/form.php" class="btn btn-success" style="background-color: #869B8F; border-color:rgb(255, 255, 255); color: #fff;">Crear Nuevo Inventario</a>
    </div>
        ...
        <?php if (!empty($inventarios)): ?>
          <?php foreach ($inventarios as $inventario): ?>
            <tr>
              ...
              <td>
                <a href="/DrVet/Views/inventario/form.php?id=<?= $inventario['ID_INVENTARIO'] ?>" class="btn btn-primary btn-sm">Editar</a>
                <a href="/DrVet/Views/inventario/delete.php?id=<?= $inventario['ID_INVENTARIO'] ?>" class="btn btn-danger btn-sm" onclick="return confirm('¿Estás seguro de que deseas eliminar este inventario?');">Eliminar</a>
              </td>
            </tr>
          <?php endforeach; ?>
        <?php else: ?>
