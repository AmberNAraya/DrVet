...
<body class="homepage bg-light">
  ...
  <div class="p-4 shadow-sm" style="background-color: #ADB8B2; border-radius: 15px;">
    <!-- Título -->
    <h1 class="text-center mb-4" style="color:rgb(42, 42, 42); font-weight: bold;">Listado de Medicamentos</h1>

    <!-- Botón Crear Medicamento -->
    <div class="text-end mb-3">
      <a href="/DrVet/Views/medicamento/form.php" class="btn btn-success" style="background-color: #869B8F; border-color:rgb(255, 255, 255); color: #fff;">Crear Nuevo Medicamento</a>
    </div>
        ...
        <?php if (!empty($medicamentos)): ?>
          <?php foreach ($medicamentos as $medicamento): ?>
            <tr>
              ...
              <td>
                <a href="/DrVet/Views/medicamento/form.php?id=<?= $medicamento['ID_MEDICAMENTO'] ?>" class="btn btn-primary btn-sm">Editar</a>
                <a href="/DrVet/Views/medicamento/delete.php?id=<?= $medicamento['ID_MEDICAMENTO'] ?>" class="btn btn-danger btn-sm" onclick="return confirm('¿Estás seguro de que deseas eliminar este medicamento?');">Eliminar</a>
              </td>
            </tr>
          <?php endforeach; ?>
        <?php else: ?>
